...
class SecurityContext:
    ...
    def __init__(
        ...
        logger.info(f"Security context created for {agent_id} with {len(permissions)} permissions")
    ...
    def has_permission(self, permission: Permission) -> bool:
        ...
        if not has_perm:
            logger.warning(f"Permission denied: {self.agent_id} attempted {permission.value}")
        return has_perm
    ...
    def require_any_permission(self, *permissions: Permission) -> None:
        ...
        if not any(self.has_permission(p) for p in permissions):
            ...
            error_msg = f"Permission denied: one of {perm_names} required for {self.agent_id}"
            logger.error(error_msg)
            raise PermissionError(error_msg)
...
class PermissionChecker:
    ...
    @staticmethod
    def filter_sensitive_data(data: dict[str, Any], context: SecurityContext) -> dict[str, Any]:
