...
class EmploymentInfo(BaseModel):
    ...
    years_employed: Optional[Decimal] = Field(None, ge=0, le=60)
    monthly_income: Decimal = Field(..., gt=0, le=1000000)
    additional_income: Optional[Decimal] = Field(default=Decimal("0"), ge=0, le=1000000)

    @validator("employer_name", "job_title")
    def validate_employment_details(cls, v: Optional[str], values: dict) -> Optional[str]:
        ...
        if values.get("status") in [EmploymentStatus.EMPLOYED, EmploymentStatus.SELF_EMPLOYED]:
            if not v:
                raise ValueError(
                    "Employer name and job title required for employed applicants"
                )
        return v
...
class FinancialInfo(BaseModel):
    ...
    checking_balance: Optional[Decimal] = Field(None, ge=0)
    savings_balance: Optional[Decimal] = Field(None, ge=0)
    investment_balance: Optional[Decimal] = Field(None, ge=0)
    ...
    def validate_bankruptcy_date(cls, v: Optional[date], values: dict) -> Optional[date]:
    ...
    def validate_foreclosure_date(cls, v: Optional[date], values: dict) -> Optional[date]:
...
class CreditHistory(BaseModel):
    ...
    oldest_credit_line_years: Optional[Decimal] = Field(None, ge=0, le=80)
...
class LoanDetails(BaseModel):
    ...
    down_payment: Optional[Decimal] = Field(default=Decimal("0"), ge=0)

    @validator("property_value")
    def validate_property_value(cls, v: Optional[Decimal], values: dict) -> Optional[Decimal]:
    ...
    def validate_down_payment(cls, v: Optional[Decimal], values: dict) -> Optional[Decimal]:
...
class LoanDecision(BaseModel):
    ...
    def validate_disapproval_reason(cls, v: Optional[str], values: dict) -> Optional[str]:
    ...
    def validate_additional_info(cls, v: Optional[str], values: dict) -> Optional[str]:
